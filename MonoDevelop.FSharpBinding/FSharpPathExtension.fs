        ...
        ast |> Option.iter (fun ast ->
            ...
            for top in topLevelTypesInsideCursor do
                ...
                if name.Contains(".") then
                    ...
                    newPath.Add(PathEntry(ImageService.GetIcon(ServiceUtils.getIcon top.Declaration, Gtk.IconSize.Menu), x.GetEntityMarkup(top.Declaration), Tag = (ast, nameparts)))
                else newPath.Add(PathEntry(ImageService.GetIcon(ServiceUtils.getIcon top.Declaration, Gtk.IconSize.Menu), x.GetEntityMarkup(top.Declaration), Tag = ast))
            ...
            if topLevelTypesInsideCursor.Length > 0 then
                ...
                match child with
                | Some(c) -> newPath.Add(PathEntry(ImageService.GetIcon(ServiceUtils.getIcon c, Gtk.IconSize.Menu), x.GetEntityMarkup(c) , Tag = lastToplevel))
