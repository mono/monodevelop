...
fragment    HEXCHAR     : ('a'..'f'|'A'..'F'|'0'..'9') ; //[a-f]|[A-F]|[0-9]   ;
...
//              grammar a lot.
//
COMMENT         : '/*' .*? '*/' -> channel(2) // Comments on channel 2 in case we want to find them
                ;
...
//                      it from the ANLTR parser.
//
CDC             : '-->' -> channel(3) // CDC on channel 4 in case we want it later                 
                ;
