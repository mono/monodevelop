    ...
    class PopupAgent : ISpaceReservationAgent
	{
		...
		public Geometry PositionAndDisplay (Geometry reservedSpace)
		{
			...
			if (visualSpan.Length > 0) {
				...
				if (left < right) {
					spanRectangle = new Rectangle (left, top, right - left, bottom - top);
				}
			}
			else {
				...
				if (line != null) {
					...
					spanRectangle = new Rectangle (bounds.Left, bounds.TextTop, 0.0, bounds.TextHeight);
				}
			}

			if (spanRectangle.HasValue && !spanRectangle.Value.IsEmpty) {
				//Get the portion of the span geometry that is inside the view.
				Rectangle viewRect = new Rectangle (_textView.ViewportLeft, _textView.ViewportTop, _textView.ViewportWidth, _textView.ViewportHeight);

				Rectangle spanRect = spanRectangle.Value;
				...
				if (!spanRect.IsEmpty) {
					...
					Rectangle spanRectWithBuffer = new Rectangle (spanRect.Left, spanRect.Top, spanRect.Right - spanRect.Left, spanRect.Bottom - spanRect.Top + BelowTheLineBufferHint);

					//Some of the text associated with the popup is visible, show the popup.
					Rectangle spanRectInScreenCoordinates = new Rectangle (this.GetScreenPointFromTextXY (spanRect.Left, spanRect.Top),
																this.GetScreenPointFromTextXY (spanRect.Right, spanRect.Bottom));
					Rectangle spanRectWithBufferInScreenCoordinates = new Rectangle (this.GetScreenPointFromTextXY (spanRectWithBuffer.Left, spanRectWithBuffer.Top),
																		  this.GetScreenPointFromTextXY (spanRectWithBuffer.Right, spanRectWithBuffer.Bottom));
					Rectangle screenRect = Xwt.Desktop.GetScreenAtLocation (spanRectInScreenCoordinates.TopLeft).Bounds;//TODO: Check if we should use VisualBounds
					...
					Rectangle reservedRect = reservedSpace.Bounds;
					...
					reservedRect = new Rectangle (topLeft, bottomRight);
					...
					Tuple<PopupStyles, Rectangle>[] positionChoices;
					if (!reservedRect.IsEmpty) {
						if ((_style & PopupStyles.PositionClosest) == 0) {
							positionChoices = new Tuple<PopupStyles, Rectangle>[]
							{
								new Tuple<PopupStyles,Rectangle>(_style, spanRectInScreenCoordinates),
								new Tuple<PopupStyles,Rectangle>(_style, spanRectWithBufferInScreenCoordinates),
								new Tuple<PopupStyles,Rectangle>(_style, reservedRect),
								new Tuple<PopupStyles,Rectangle>(alternateStyle, spanRectInScreenCoordinates),
								new Tuple<PopupStyles,Rectangle>(alternateStyle, spanRectWithBufferInScreenCoordinates),
								new Tuple<PopupStyles,Rectangle>(alternateStyle, reservedRect),
							};
						}
						else {
							positionChoices = new Tuple<PopupStyles, Rectangle>[]
					...
					Rectangle location = Rectangle.Zero;
					foreach (var choice in positionChoices) {
						Rectangle locationToTry = GetLocation (choice.Item1, desiredSize, spanRectInScreenCoordinates, choice.Item2, screenRect);
		...
		internal bool ShouldClearToolTipOnMouseMove (Point mousePt)
		{
			...
			if (!new Rectangle (topLeft.ToXwtPoint (), bottomRight.ToXwtPoint ()).Contains (x, y))
				return true;
		...
		internal static Rectangle GetLocation (PopupStyles style, Size desiredSize, Rectangle spanRectInScreenCoordinates, Rectangle reservedRect, Rectangle screenRect)
		{
		...
		internal static Rectangle ShiftHorizontallyToFitScreen (double x, double y, Size desiredSize, Rectangle screenRect)
		{
			...
			return new Rectangle (x, y, desiredSize.Width, desiredSize.Height);
		}

		internal static Rectangle ShiftVerticallyToFitScreen (double x, double y, Size desiredSize, Rectangle screenRect)
		{
		...
		internal static bool DisjointWithPadding (Geometry reserved, Rectangle location)
		{
			...
			if ((width > 0.0) && (height > 0.0)) {
				Geometry insetLocation = new RectangleGeometry (new Rectangle (left, top, width, height));
		...
		internal static bool ContainsWithPadding (Rectangle outer, Rectangle inner)
		{
