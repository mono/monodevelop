...
namespace MonoDevelop.VersionControl.Subversion.Unix
{
	...
	sealed class UnixSvnBackend : SubversionBackend
	{
		...
		public override IEnumerable<DirectoryEntry> ListUrl (string url, bool recurse, SvnRevision rev)
		{
			...
			IntPtr localpool = TryStartOperation (null, InOperationType.ListUrl);
		...
		public override IEnumerable<VersionInfo> Status (Repository repo, FilePath path, SvnRevision rev, bool descendDirs, bool changedItemsOnly, bool remoteStatus)
		{
			...
			var localpool = TryStartOperation (null, InOperationType.Status);
			try {
		...
		public override IEnumerable<SvnRevision> Log (Repository repo, FilePath path, SvnRevision revStart, SvnRevision revEnd)
		{
			...
			var localpool = TryStartOperation (null, InOperationType.Log);
			try {
		...
		public override Annotation[] GetAnnotations (Repository repo, FilePath file, SvnRevision revStart, SvnRevision revEnd)
		{
			...
			var localpool = TryStartOperation (null, InOperationType.Annotation);
			try {
		...
		public void Cat (string pathorurl, SvnRevision rev, Stream stream)
		{
			...
			var localpool = TryStartOperation (null, InOperationType.Cat);
			try {
		...
		public override void Update (FilePath path, bool recurse, IProgressMonitor monitor)
		{
			...
			var localpool = TryStartOperation (monitor, InOperationType.Update);
			try {
				string pathorurl = NormalizePath (path, localpool);
		...
		public override void Revert (FilePath[] paths, bool recurse, IProgressMonitor monitor)
		{
			...
			var localpool = TryStartOperation (monitor, InOperationType.Revert);
			try {
		...
		public override void Add (FilePath path, bool recurse, IProgressMonitor monitor)
		{
			...
			nb = new notify_baton ();
			var localpool = TryStartOperation (monitor, InOperationType.Add);
			try {
				string pathorurl = NormalizePath (path, localpool);
		...
		public override void Checkout (string url, FilePath path, Revision revision, bool recurse, IProgressMonitor monitor)
		{
			...
			nb = new notify_baton ();
			var localpool = TryStartOperation (monitor, InOperationType.Checkout);
			try {
		...
		public override void Commit (FilePath[] paths, string message, IProgressMonitor monitor)
		{
			...
			nb = new notify_baton ();
			var localpool = TryStartOperation (monitor, InOperationType.Commit);
			try {
				// Put each item into an APR array.
				IntPtr array = apr.array_make (localpool, 0, IntPtr.Size);
				foreach (string path in paths) {
		...
		public override void Mkdir (string[] paths, string message, IProgressMonitor monitor) 
		{
			...
			nb = new notify_baton ();
			var localpool = TryStartOperation (monitor, InOperationType.Mkdir);
			try {
				// Put each item into an APR array.
		...
		public override void Delete (FilePath path, bool force, IProgressMonitor monitor)
		{
			...
			nb = new notify_baton ();
			var localpool = TryStartOperation (monitor, InOperationType.Delete);
			try {
				// Put each item into an APR array.
				IntPtr array = apr.array_make (localpool, 0, IntPtr.Size);
				//foreach (string path in paths) {
		...
		public override void Move (FilePath srcPath, FilePath destPath, SvnRevision rev, bool force, IProgressMonitor monitor)
		{
			...
			nb = new notify_baton ();
			IntPtr commit_info = IntPtr.Zero;
			var localpool = TryStartOperation (monitor, InOperationType.Move);
			try {
		...
		{
			nb = new notify_baton ();
			var localpool = TryStartOperation (monitor, InOperationType.Lock);
			try {
				IntPtr array = apr.array_make (localpool, 0, IntPtr.Size);
				foreach (string path in paths) {
					string npath = NormalizePath (path, localpool);
		...
		{
			nb = new notify_baton ();
			var localpool = TryStartOperation (monitor, InOperationType.Unlock);
			try {
				IntPtr array = apr.array_make (localpool, 0, IntPtr.Size);
				foreach (string path in paths) {
					string npath = NormalizePath (path, localpool);
		...
		public override string GetUnifiedDiff (FilePath path1, SvnRevision rev1, FilePath path2, SvnRevision rev2, bool recursive)
		{
			...
			var localpool = TryStartOperation (null, InOperationType.Diff);
			try {
		...
		{
			var localpool = TryStartOperation (null, InOperationType.Merge);
			try {
		...
		public override void Ignore (FilePath[] paths)
		{
			...
			var localpool = TryStartOperation (null, InOperationType.Ignore);
			try {
		...
		public override void Unignore (FilePath[] paths)
		{
			...
			var localpool = TryStartOperation (null, InOperationType.Unignore);
			try {
				foreach (var path in paths) {
					new_path = NormalizePath (path, localpool);
					CheckError (svn.client_propget (out result, "svn:ignore", Path.GetDirectoryName (new_path),
		...
		IntPtr TryStartOperation (IProgressMonitor monitor, InOperationType opType)
		{
			lock (sync) {
				if (inProgress != InOperationType.None)
					throw new SubversionException (String.Format ("Can't start {0}. {1} Subversion operation is in progress.", opType, inProgress));
				inProgress = opType;
		...
		void TryEndOperation ()
		{
			lock (sync) {
				...
				inProgress = InOperationType.None;
				updatemonitor = null;
			}
		}
...
//				break;
		...
		internal string GetDirectoryDotSvnInternal (FilePath path)
		{
			...
			var localpool = TryStartOperation (null, InOperationType.GetDirectory);
			try {
