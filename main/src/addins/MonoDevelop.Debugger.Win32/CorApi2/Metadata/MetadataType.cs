...
namespace Microsoft.Samples.Debugging.CorMetadata
{
	public sealed class MetadataType : Type
	{
		...
		static string GetTypeName (IMetadataImport importer, int tk)
		{
		...
		private static CorElementType GetEnumUnderlyingType (IMetadataImport importer, int tk)
		{
			...
			{
				importer.GetFieldProps (mdFieldDef, out classToken, null, 0, out nameSize, out fieldAttributes, out ppvSig,
					out size, out cPlusTypeFlab, out ppValue, out pcchValue);
					...
					for (int i = 0; i < fields.Length; i++)
					{
						...
						if ((field.Attributes & staticLiteralField) == staticLiteralField)
						{
							m_enumValues.Add (new KeyValuePair<string, ulong> (field.Name,
								Convert.ToUInt64 (field.GetValue (null), CultureInfo.InvariantCulture)));
						}
					}
