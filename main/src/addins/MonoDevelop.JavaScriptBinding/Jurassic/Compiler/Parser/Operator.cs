...
namespace Jurassic.Compiler
{
	...
	public sealed class Operator
	{
		...
		private Operator (Token token, int precedence, OperatorPlacement placement, OperatorAssociativity associativity, OperatorType type, Token secondaryToken = null, int rhsPrecedence = -1)
		{
			...
			Token = token;
			HasLHSOperand = (placement & OperatorPlacement.HasLHSOperand) != 0;
			HasRHSOperand = (placement & OperatorPlacement.HasRHSOperand) != 0;
			Associativity = associativity;
			Type = type;
			SecondaryToken = secondaryToken;
			HasSecondaryRHSOperand = (placement & OperatorPlacement.HasSecondaryRHSOperand) != 0;
			InnerOperandIsOptional = (placement & OperatorPlacement.InnerOperandIsOptional) != 0;
			Precedence = SecondaryPrecedence = TertiaryPrecedence = precedence;
			if (rhsPrecedence >= 0 && HasRHSOperand)
				SecondaryPrecedence = rhsPrecedence;
			if (rhsPrecedence >= 0 && HasSecondaryRHSOperand)
				TertiaryPrecedence = rhsPrecedence;
		...
		/// </summary>
		public int Arity {
			get { return (HasLHSOperand ? 1 : 0) + (HasRHSOperand ? 1 : 0) + (HasSecondaryRHSOperand ? 1 : 0); }
		}
		...
		public bool IsValidNumberOfOperands (int operandCount)
		{
			if (InnerOperandIsOptional)
				return operandCount == ((HasLHSOperand ? 1 : 0) + 0 + (HasSecondaryRHSOperand ? 1 : 0)) ||
				operandCount == ((HasLHSOperand ? 1 : 0) + 1 + (HasSecondaryRHSOperand ? 1 : 0));
			return operandCount == (HasLHSOperand ? 1 : 0) + (HasRHSOperand ? 1 : 0) + (HasSecondaryRHSOperand ? 1 : 0);
		}
