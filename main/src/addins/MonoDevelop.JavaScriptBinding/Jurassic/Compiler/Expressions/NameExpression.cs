	...
	public sealed class NameExpression : Expression, IReferenceExpression
	{
		...
		public override PrimitiveType ResultType {
			get {
				...
				if (!Scope.ExistsAtRuntime) {
					var scope = Scope;
					do {
						var variableInfo = Scope.GetDeclaredVariable (Name);
		...
		{
			return Name.GetHashCode () ^ Scope.GetHashCode ();
		}
		...
		public override bool Equals (object obj)
		{
			if (!(obj is NameExpression))
				return false;
			return Name == ((NameExpression)obj).Name && Scope == ((NameExpression)obj).Scope;
		}
