...
namespace Jurassic
{
	...
	public struct BigInteger
	{
		...
		BigInteger (uint[] bits, int wordCount, int sign)
		{
			this.bits = bits;
		...
		public BigInteger (long value)
		{
			bits = new uint[2];
			bits [0] = (uint)Math.Abs (value);
			bits [1] = (uint)(Math.Abs (value) >> 32);
			wordCount = bits [1] != 0 ? 2 : 1;
			sign = Math.Sign (value);
		}
		...
		public uint HighWord {
			get { return bits [wordCount - 1]; }
		}

		public int BitCount {
			get { return (32 - CountLeadingZeroBits (bits [wordCount - 1])) + (wordCount - 1) * 32; }
		}
		...
		static readonly int[] powersOfFive = { 5, 25, 125 };
		...
		static readonly long[] integerPowersOfTen = new long[] {
			1L,
			10L,
			100L,
			1000L,
		...
		public override bool Equals (object obj)
		{
			if (!(obj is BigInteger))
				return false;
			if (wordCount != ((BigInteger)obj).wordCount)
				return false;
