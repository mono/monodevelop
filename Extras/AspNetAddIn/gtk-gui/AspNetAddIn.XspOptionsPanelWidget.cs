// ------------------------------------------------------------------------------
//  <autogenerated>
//      This code was generated by a tool.
//      Mono Runtime Version: 2.0.50727.42
// 
//      Changes to this file may cause incorrect behavior and will be lost if 
//      the code is regenerated.
//  </autogenerated>
// ------------------------------------------------------------------------------

namespace AspNetAddIn {
    
    
    public partial class XspOptionsPanelWidget {
        
        private Gtk.VBox vbox1;
        
        private Gtk.Label label1;
        
        private Gtk.HBox hbox2;
        
        private Gtk.Label label4;
        
        private Gtk.VBox vbox2;
        
        private Gtk.Table table2;
        
        private Gtk.Entry ipAddress;
        
        private Gtk.Label label7;
        
        private Gtk.Label label8;
        
        private Gtk.SpinButton portNumber;
        
        private Gtk.CheckButton verboseCheck;
        
        private Gtk.Label label2;
        
        private Gtk.HBox hbox3;
        
        private Gtk.Label label5;
        
        private Gtk.Table table3;
        
        private Gtk.Label label10;
        
        private Gtk.Label label9;
        
        private Gtk.ComboBox sslMode;
        
        private Gtk.ComboBox sslProtocol;
        
        private Gtk.Label label3;
        
        private Gtk.HBox hbox4;
        
        private Gtk.Label label6;
        
        private Gtk.Table table4;
        
        private MonoDevelop.Components.FileEntry certLocation;
        
        private Gtk.HBox hbox5;
        
        private Gtk.ComboBox keyType;
        
        private Gtk.HBox hbox6;
        
        private Gtk.ComboBox passwordOptions;
        
        private Gtk.Entry passwordEntry;
        
        private MonoDevelop.Components.FileEntry keyLocation;
        
        private Gtk.Label label11;
        
        private Gtk.Label label12;
        
        private Gtk.Label label13;
        
        private Gtk.Label label14;
        
        protected virtual void Build() {
            Stetic.Gui.Initialize(this);
            // Widget AspNetAddIn.XspOptionsPanelWidget
            Stetic.BinContainer.Attach(this);
            this.Name = "AspNetAddIn.XspOptionsPanelWidget";
            // Container child AspNetAddIn.XspOptionsPanelWidget.Gtk.Container+ContainerChild
            this.vbox1 = new Gtk.VBox();
            this.vbox1.Name = "vbox1";
            this.vbox1.Spacing = 12;
            // Container child vbox1.Gtk.Box+BoxChild
            this.label1 = new Gtk.Label();
            this.label1.Name = "label1";
            this.label1.Xalign = 0.01F;
            this.label1.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("<b>General Options</b>");
            this.label1.UseMarkup = true;
            this.vbox1.Add(this.label1);
            Gtk.Box.BoxChild w1 = ((Gtk.Box.BoxChild)(this.vbox1[this.label1]));
            w1.Position = 0;
            w1.Expand = false;
            w1.Fill = false;
            // Container child vbox1.Gtk.Box+BoxChild
            this.hbox2 = new Gtk.HBox();
            this.hbox2.Name = "hbox2";
            // Container child hbox2.Gtk.Box+BoxChild
            this.label4 = new Gtk.Label();
            this.label4.WidthRequest = 18;
            this.label4.Name = "label4";
            this.label4.LabelProp = "";
            this.hbox2.Add(this.label4);
            Gtk.Box.BoxChild w2 = ((Gtk.Box.BoxChild)(this.hbox2[this.label4]));
            w2.Position = 0;
            w2.Expand = false;
            w2.Fill = false;
            // Container child hbox2.Gtk.Box+BoxChild
            this.vbox2 = new Gtk.VBox();
            this.vbox2.Name = "vbox2";
            this.vbox2.Spacing = 6;
            // Container child vbox2.Gtk.Box+BoxChild
            this.table2 = new Gtk.Table(((uint)(2)), ((uint)(2)), false);
            this.table2.Name = "table2";
            this.table2.RowSpacing = ((uint)(6));
            this.table2.ColumnSpacing = ((uint)(6));
            // Container child table2.Gtk.Table+TableChild
            this.ipAddress = new Gtk.Entry();
            this.ipAddress.CanFocus = true;
            this.ipAddress.Name = "ipAddress";
            this.ipAddress.IsEditable = true;
            this.ipAddress.InvisibleChar = '●';
            this.table2.Add(this.ipAddress);
            Gtk.Table.TableChild w3 = ((Gtk.Table.TableChild)(this.table2[this.ipAddress]));
            w3.LeftAttach = ((uint)(1));
            w3.RightAttach = ((uint)(2));
            w3.XOptions = ((Gtk.AttachOptions)(4));
            w3.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table2.Gtk.Table+TableChild
            this.label7 = new Gtk.Label();
            this.label7.Name = "label7";
            this.label7.Xalign = 0F;
            this.label7.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("Port number:");
            this.table2.Add(this.label7);
            Gtk.Table.TableChild w4 = ((Gtk.Table.TableChild)(this.table2[this.label7]));
            w4.TopAttach = ((uint)(1));
            w4.BottomAttach = ((uint)(2));
            w4.XOptions = ((Gtk.AttachOptions)(4));
            w4.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table2.Gtk.Table+TableChild
            this.label8 = new Gtk.Label();
            this.label8.Name = "label8";
            this.label8.Xalign = 0F;
            this.label8.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("IP address:");
            this.table2.Add(this.label8);
            Gtk.Table.TableChild w5 = ((Gtk.Table.TableChild)(this.table2[this.label8]));
            w5.XOptions = ((Gtk.AttachOptions)(4));
            w5.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table2.Gtk.Table+TableChild
            this.portNumber = new Gtk.SpinButton(0, 100, 1);
            this.portNumber.CanFocus = true;
            this.portNumber.Name = "portNumber";
            this.portNumber.Adjustment.PageIncrement = 10;
            this.portNumber.ClimbRate = 1;
            this.portNumber.Numeric = true;
            this.table2.Add(this.portNumber);
            Gtk.Table.TableChild w6 = ((Gtk.Table.TableChild)(this.table2[this.portNumber]));
            w6.TopAttach = ((uint)(1));
            w6.BottomAttach = ((uint)(2));
            w6.LeftAttach = ((uint)(1));
            w6.RightAttach = ((uint)(2));
            w6.XOptions = ((Gtk.AttachOptions)(4));
            w6.YOptions = ((Gtk.AttachOptions)(4));
            this.vbox2.Add(this.table2);
            Gtk.Box.BoxChild w7 = ((Gtk.Box.BoxChild)(this.vbox2[this.table2]));
            w7.Position = 0;
            w7.Expand = false;
            w7.Fill = false;
            // Container child vbox2.Gtk.Box+BoxChild
            this.verboseCheck = new Gtk.CheckButton();
            this.verboseCheck.CanFocus = true;
            this.verboseCheck.Name = "verboseCheck";
            this.verboseCheck.Label = MonoDevelop.Core.GettextCatalog.GetString("Verbose console output");
            this.verboseCheck.DrawIndicator = true;
            this.verboseCheck.UseUnderline = true;
            this.vbox2.Add(this.verboseCheck);
            Gtk.Box.BoxChild w8 = ((Gtk.Box.BoxChild)(this.vbox2[this.verboseCheck]));
            w8.Position = 1;
            w8.Expand = false;
            w8.Fill = false;
            this.hbox2.Add(this.vbox2);
            Gtk.Box.BoxChild w9 = ((Gtk.Box.BoxChild)(this.hbox2[this.vbox2]));
            w9.Position = 1;
            w9.Expand = false;
            w9.Fill = false;
            this.vbox1.Add(this.hbox2);
            Gtk.Box.BoxChild w10 = ((Gtk.Box.BoxChild)(this.vbox1[this.hbox2]));
            w10.Position = 1;
            w10.Expand = false;
            w10.Fill = false;
            // Container child vbox1.Gtk.Box+BoxChild
            this.label2 = new Gtk.Label();
            this.label2.Name = "label2";
            this.label2.Xalign = 0.01F;
            this.label2.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("<b>Security</b>");
            this.label2.UseMarkup = true;
            this.vbox1.Add(this.label2);
            Gtk.Box.BoxChild w11 = ((Gtk.Box.BoxChild)(this.vbox1[this.label2]));
            w11.Position = 2;
            w11.Expand = false;
            w11.Fill = false;
            // Container child vbox1.Gtk.Box+BoxChild
            this.hbox3 = new Gtk.HBox();
            this.hbox3.Name = "hbox3";
            // Container child hbox3.Gtk.Box+BoxChild
            this.label5 = new Gtk.Label();
            this.label5.WidthRequest = 18;
            this.label5.Name = "label5";
            this.label5.LabelProp = "";
            this.hbox3.Add(this.label5);
            Gtk.Box.BoxChild w12 = ((Gtk.Box.BoxChild)(this.hbox3[this.label5]));
            w12.Position = 0;
            w12.Expand = false;
            w12.Fill = false;
            // Container child hbox3.Gtk.Box+BoxChild
            this.table3 = new Gtk.Table(((uint)(2)), ((uint)(2)), false);
            this.table3.Name = "table3";
            this.table3.RowSpacing = ((uint)(6));
            this.table3.ColumnSpacing = ((uint)(6));
            // Container child table3.Gtk.Table+TableChild
            this.label10 = new Gtk.Label();
            this.label10.Name = "label10";
            this.label10.Xalign = 0F;
            this.label10.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("SSL mode:");
            this.table3.Add(this.label10);
            Gtk.Table.TableChild w13 = ((Gtk.Table.TableChild)(this.table3[this.label10]));
            w13.XOptions = ((Gtk.AttachOptions)(4));
            w13.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table3.Gtk.Table+TableChild
            this.label9 = new Gtk.Label();
            this.label9.Name = "label9";
            this.label9.Xalign = 0F;
            this.label9.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("SSL protocol:");
            this.table3.Add(this.label9);
            Gtk.Table.TableChild w14 = ((Gtk.Table.TableChild)(this.table3[this.label9]));
            w14.TopAttach = ((uint)(1));
            w14.BottomAttach = ((uint)(2));
            w14.XOptions = ((Gtk.AttachOptions)(4));
            w14.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table3.Gtk.Table+TableChild
            this.sslMode = Gtk.ComboBox.NewText();
            this.sslMode.Name = "sslMode";
            this.table3.Add(this.sslMode);
            Gtk.Table.TableChild w15 = ((Gtk.Table.TableChild)(this.table3[this.sslMode]));
            w15.LeftAttach = ((uint)(1));
            w15.RightAttach = ((uint)(2));
            w15.XOptions = ((Gtk.AttachOptions)(4));
            w15.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table3.Gtk.Table+TableChild
            this.sslProtocol = Gtk.ComboBox.NewText();
            this.sslProtocol.Name = "sslProtocol";
            this.table3.Add(this.sslProtocol);
            Gtk.Table.TableChild w16 = ((Gtk.Table.TableChild)(this.table3[this.sslProtocol]));
            w16.TopAttach = ((uint)(1));
            w16.BottomAttach = ((uint)(2));
            w16.LeftAttach = ((uint)(1));
            w16.RightAttach = ((uint)(2));
            w16.XOptions = ((Gtk.AttachOptions)(4));
            w16.YOptions = ((Gtk.AttachOptions)(4));
            this.hbox3.Add(this.table3);
            Gtk.Box.BoxChild w17 = ((Gtk.Box.BoxChild)(this.hbox3[this.table3]));
            w17.Position = 1;
            w17.Expand = false;
            w17.Fill = false;
            this.vbox1.Add(this.hbox3);
            Gtk.Box.BoxChild w18 = ((Gtk.Box.BoxChild)(this.vbox1[this.hbox3]));
            w18.Position = 3;
            w18.Expand = false;
            w18.Fill = false;
            // Container child vbox1.Gtk.Box+BoxChild
            this.label3 = new Gtk.Label();
            this.label3.Name = "label3";
            this.label3.Xalign = 0.01F;
            this.label3.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("<b>SSL Key</b>");
            this.label3.UseMarkup = true;
            this.vbox1.Add(this.label3);
            Gtk.Box.BoxChild w19 = ((Gtk.Box.BoxChild)(this.vbox1[this.label3]));
            w19.Position = 4;
            w19.Expand = false;
            w19.Fill = false;
            // Container child vbox1.Gtk.Box+BoxChild
            this.hbox4 = new Gtk.HBox();
            this.hbox4.Name = "hbox4";
            // Container child hbox4.Gtk.Box+BoxChild
            this.label6 = new Gtk.Label();
            this.label6.WidthRequest = 18;
            this.label6.Name = "label6";
            this.label6.LabelProp = "";
            this.hbox4.Add(this.label6);
            Gtk.Box.BoxChild w20 = ((Gtk.Box.BoxChild)(this.hbox4[this.label6]));
            w20.Position = 0;
            w20.Expand = false;
            w20.Fill = false;
            // Container child hbox4.Gtk.Box+BoxChild
            this.table4 = new Gtk.Table(((uint)(4)), ((uint)(2)), false);
            this.table4.Name = "table4";
            this.table4.RowSpacing = ((uint)(6));
            this.table4.ColumnSpacing = ((uint)(6));
            // Container child table4.Gtk.Table+TableChild
            this.certLocation = new MonoDevelop.Components.FileEntry();
            this.certLocation.Name = "certLocation";
            this.table4.Add(this.certLocation);
            Gtk.Table.TableChild w21 = ((Gtk.Table.TableChild)(this.table4[this.certLocation]));
            w21.TopAttach = ((uint)(2));
            w21.BottomAttach = ((uint)(3));
            w21.LeftAttach = ((uint)(1));
            w21.RightAttach = ((uint)(2));
            w21.XOptions = ((Gtk.AttachOptions)(4));
            w21.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table4.Gtk.Table+TableChild
            this.hbox5 = new Gtk.HBox();
            this.hbox5.Name = "hbox5";
            this.hbox5.Spacing = 6;
            // Container child hbox5.Gtk.Box+BoxChild
            this.keyType = Gtk.ComboBox.NewText();
            this.keyType.Name = "keyType";
            this.hbox5.Add(this.keyType);
            Gtk.Box.BoxChild w22 = ((Gtk.Box.BoxChild)(this.hbox5[this.keyType]));
            w22.Position = 0;
            w22.Expand = false;
            w22.Fill = false;
            this.table4.Add(this.hbox5);
            Gtk.Table.TableChild w23 = ((Gtk.Table.TableChild)(this.table4[this.hbox5]));
            w23.LeftAttach = ((uint)(1));
            w23.RightAttach = ((uint)(2));
            w23.XOptions = ((Gtk.AttachOptions)(4));
            w23.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table4.Gtk.Table+TableChild
            this.hbox6 = new Gtk.HBox();
            this.hbox6.Name = "hbox6";
            this.hbox6.Spacing = 6;
            // Container child hbox6.Gtk.Box+BoxChild
            this.passwordOptions = Gtk.ComboBox.NewText();
            this.passwordOptions.Name = "passwordOptions";
            this.hbox6.Add(this.passwordOptions);
            Gtk.Box.BoxChild w24 = ((Gtk.Box.BoxChild)(this.hbox6[this.passwordOptions]));
            w24.Position = 0;
            w24.Expand = false;
            w24.Fill = false;
            // Container child hbox6.Gtk.Box+BoxChild
            this.passwordEntry = new Gtk.Entry();
            this.passwordEntry.CanFocus = true;
            this.passwordEntry.Name = "passwordEntry";
            this.passwordEntry.IsEditable = true;
            this.passwordEntry.InvisibleChar = '●';
            this.hbox6.Add(this.passwordEntry);
            Gtk.Box.BoxChild w25 = ((Gtk.Box.BoxChild)(this.hbox6[this.passwordEntry]));
            w25.Position = 1;
            this.table4.Add(this.hbox6);
            Gtk.Table.TableChild w26 = ((Gtk.Table.TableChild)(this.table4[this.hbox6]));
            w26.TopAttach = ((uint)(3));
            w26.BottomAttach = ((uint)(4));
            w26.LeftAttach = ((uint)(1));
            w26.RightAttach = ((uint)(2));
            w26.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table4.Gtk.Table+TableChild
            this.keyLocation = new MonoDevelop.Components.FileEntry();
            this.keyLocation.Name = "keyLocation";
            this.table4.Add(this.keyLocation);
            Gtk.Table.TableChild w27 = ((Gtk.Table.TableChild)(this.table4[this.keyLocation]));
            w27.TopAttach = ((uint)(1));
            w27.BottomAttach = ((uint)(2));
            w27.LeftAttach = ((uint)(1));
            w27.RightAttach = ((uint)(2));
            w27.XOptions = ((Gtk.AttachOptions)(4));
            w27.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table4.Gtk.Table+TableChild
            this.label11 = new Gtk.Label();
            this.label11.Name = "label11";
            this.label11.Xalign = 0.01F;
            this.label11.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("Key type:");
            this.table4.Add(this.label11);
            Gtk.Table.TableChild w28 = ((Gtk.Table.TableChild)(this.table4[this.label11]));
            w28.XOptions = ((Gtk.AttachOptions)(4));
            w28.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table4.Gtk.Table+TableChild
            this.label12 = new Gtk.Label();
            this.label12.Name = "label12";
            this.label12.Xalign = 0F;
            this.label12.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("Key location:");
            this.table4.Add(this.label12);
            Gtk.Table.TableChild w29 = ((Gtk.Table.TableChild)(this.table4[this.label12]));
            w29.TopAttach = ((uint)(1));
            w29.BottomAttach = ((uint)(2));
            w29.XOptions = ((Gtk.AttachOptions)(4));
            w29.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table4.Gtk.Table+TableChild
            this.label13 = new Gtk.Label();
            this.label13.Name = "label13";
            this.label13.Xalign = 0F;
            this.label13.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("Certificate location:");
            this.table4.Add(this.label13);
            Gtk.Table.TableChild w30 = ((Gtk.Table.TableChild)(this.table4[this.label13]));
            w30.TopAttach = ((uint)(2));
            w30.BottomAttach = ((uint)(3));
            w30.XOptions = ((Gtk.AttachOptions)(4));
            w30.YOptions = ((Gtk.AttachOptions)(4));
            // Container child table4.Gtk.Table+TableChild
            this.label14 = new Gtk.Label();
            this.label14.Name = "label14";
            this.label14.Xalign = 0F;
            this.label14.LabelProp = MonoDevelop.Core.GettextCatalog.GetString("Password:");
            this.table4.Add(this.label14);
            Gtk.Table.TableChild w31 = ((Gtk.Table.TableChild)(this.table4[this.label14]));
            w31.TopAttach = ((uint)(3));
            w31.BottomAttach = ((uint)(4));
            w31.XOptions = ((Gtk.AttachOptions)(4));
            w31.YOptions = ((Gtk.AttachOptions)(4));
            this.hbox4.Add(this.table4);
            Gtk.Box.BoxChild w32 = ((Gtk.Box.BoxChild)(this.hbox4[this.table4]));
            w32.Position = 1;
            this.vbox1.Add(this.hbox4);
            Gtk.Box.BoxChild w33 = ((Gtk.Box.BoxChild)(this.vbox1[this.hbox4]));
            w33.Position = 5;
            w33.Expand = false;
            w33.Fill = false;
            this.Add(this.vbox1);
            if ((this.Child != null)) {
                this.Child.ShowAll();
            }
            this.Show();
            this.sslProtocol.Changed += new System.EventHandler(this.updateSensitivity);
            this.sslMode.Changed += new System.EventHandler(this.updateSensitivity);
            this.passwordOptions.Changed += new System.EventHandler(this.updateSensitivity);
            this.keyType.Changed += new System.EventHandler(this.updateSensitivity);
        }
    }
}
